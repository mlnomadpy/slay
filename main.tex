...
\icmltitlerunning{Spherical Yat-Attention}
...
We introduce \textbf{Spherical YAT-Attention}, a geometry-aware attention mechanism, with the following characteristics:
...
Table~\ref{tab:poly-approximations} summarizes the main theoretical trade-offs: feature dimension and computational cost, as well as qualitative properties such as unbiasedness, stability, and whether the approximate inner products are guaranteed nonnegative.
...
Anchor (low-rank) & $P$ & $O(d\,P)$ & No & Yes \\
\bottomrule
\end{tabular*}
\end{table*}

For the theoretical non-negativity and denominator-positivity guarantees stated later, we require a polynomial component whose induced score estimates are nonnegative (e.g., the exact map, or anchor features). Generic randomized/low-rank approximations can yield negative approximate inner products and are therefore treated as accuracy/efficiency baselines.
...
% ------------------------------------------------------------
\subsection{End-to-End Training: BERT and GPT}
\label{subsec:e2e-training}

We train base encoder (BERT-style) and decoder (GPT-style) Transformer models, swapping softmax attention for Spherical \E-Attention while keeping the rest of the architecture fixed. All runs use a maximum sequence length of $2048$.

Table~\ref{tab:param-count} shows that the random-feature projections add a negligible parameter overhead relative to the base model.

\begin{table}[t]
\centering
\small
\caption{Parameter counts for base models used in experiments.}
\label{tab:param-count}
\begin{tabular}{lrr}
\toprule
Model & Standard & Spherical $\E$ \\
\midrule
BERT & $152{,}596{,}992$ & $152{,}633{,}856$ \\
GPT  & $152{,}596{,}992$ & $152{,}633{,}856$ \\
\bottomrule
\end{tabular}
\end{table}

We train for one epoch with identical optimization settings and data; Table~\ref{tab:e2e-training} reports final loss and perplexity.

\begin{table}[H]
\centering
\caption{End-to-end training (1 epoch, max\_seq\_len=2048).}
\label{tab:e2e-training}
\begin{tabular}{lcc}
\toprule
Model & Loss & Perplexity \\
\midrule
BERT-Standard & 3.0537 & 21.19 \\
BERT-\E      & 3.0398 & 20.90 \\
GPT-Standard  & 2.8914 & 18.02 \\
GPT-\E       & 3.0520 & 21.16 \\
\bottomrule
\end{tabular}
\end{table}

BERT-\E matches BERT-Standard closely after one epoch (Table~\ref{tab:e2e-training}). For GPT, GPT-\E ends with higher perplexity than GPT-Standard under this short training run, consistent with approximation error from linearized attention. We did not observe optimization failures (e.g., divergence) in these runs.

% ------------------------------------------------------------
\subsection{Attention-Only Micro-Benchmarks}
\label{subsec:attn-micro}

To isolate scaling effects, we benchmark a single attention layer. Table~\ref{tab:microbench-setup} summarizes the setup.

\begin{table}[t]
\centering
\small
\caption{Attention-only micro-benchmark setup.}
\label{tab:microbench-setup}
\begin{tabular}{ll}
\toprule
Setting & Value \\
\midrule
Per-head dimension & $d_{\text{head}}=64$ \\
Batch size & $B=1$ \\
Sequence lengths & $L\in\{256,512,\ldots,32768\}$ \\
\bottomrule
\end{tabular}
\end{table}

We report average latency and peak memory for forward-only (encoder-style) and forward+backward (causal) execution.

At $L=32768$ (Table~\ref{tab:attn-latency-32768}), Spherical \E-Attention is faster than standard attention in both regimes: $2.5$ vs.~$30.6$ ms for forward-only and $8.0$ vs.~$109.0$ ms for forward+backward. This gap is consistent with avoiding the explicit $L\times L$ attention matrix and using featureization followed by linear contractions.

\begin{table}[t]
\centering
\small
\caption{Attention-only latency at $L=32768$ (ms).}
\label{tab:attn-latency-32768}
\begin{tabular}{lcc}
\toprule
Method & Forward-only & Forward+backward \\
\midrule
Standard attention & 30.6 & 109.0 \\
Spherical $\E$ attention & 2.5 & 8.0 \\
\bottomrule
\end{tabular}
\end{table}

Peak memory at $L=32768$ (Table~\ref{tab:attn-mem-32768}) is $31.0$ GB for standard attention versus $14.3$ GB for Spherical \E-Attention in the forward+backward setting. This reduction is consistent with replacing attention-matrix materialization with an $O(Lm)$ feature buffer and subsequent contractions.

\begin{table}[t]
\centering
\small
\caption{Peak memory at $L=32768$ (forward+backward).}
\label{tab:attn-mem-32768}
\begin{tabular}{lc}
\toprule
Method & Peak memory (GB) \\
\midrule
Standard attention & 31.0 \\
Spherical $\E$ attention & 14.3 \\
\bottomrule
\end{tabular}
\end{table}

Overall, these micro-benchmarks illustrate that the \E implementation remains tractable at sequence lengths where the quadratic costs of standard attention dominate.

% ------------------------------------------------------------
\subsection{Full-Model Inference Scaling}
\label{subsec:full-model}

We next benchmark full BERT and GPT models in inference mode. All models are configured with a maximum supported sequence length of $2048$.

At short contexts, standard attention is faster due to lower constant factors. Table~\ref{tab:gpt-infer-lat} shows that GPT-$\E$ has roughly constant latency across tested $L$, while GPT-Standard slows as $L$ increases. \E therefore trades short-context throughput for scalability and is most useful once sequence lengths move beyond the regime where quadratic attention is affordable.

\begin{table}[t]
\centering
\small
\caption{GPT inference latency (ms) at short vs. max-supported context.}
\label{tab:gpt-infer-lat}
\begin{tabular}{lcc}
\toprule
Model & $L=256$ & $L=2048$ \\
\midrule
GPT-Standard & 58.6 & 80.8 \\
GPT-$\E$ & \multicolumn{2}{c}{$\approx 320$ (flat across tested $L$)} \\
\bottomrule
\end{tabular}
\end{table}

% ------------------------------------------------------------
\subsection{Maximum Sequence Length under Fixed Memory}
\label{subsec:max-l}

Finally, we push GPT-\E to the largest context that fits within an $80$ GB GPU memory budget in inference mode.
...
\addlegendentry{GPT-$\E$ (inference)}
\end{axis}
\end{tikzpicture}
\caption{Peak memory vs. sequence length for GPT-$\E$ under an 80\,GB budget. Exact values (including the OOM point) are reported in Appendix~\ref{app:experiments}.}
\label{fig:max-seqlen}
\end{figure}

The largest successful sequence length is $L=65{,}536$.
...
In our implementation, $R$ controls the quadrature accuracy of the Laplace integral and $D$ controls the Monte Carlo variance of PRF. The polynomial approximation uses either a feature dimension $D_p$ (e.g., Random Maclaurin or TensorSketch) or an anchor count $P$ (anchor/Nystr"om-style maps); by default we use anchor features because they preserve non-negativity of the polynomial component and are stable at small budgets.
