...
class SLAYFeatures(nnx.Module):
    ...
    def __call__(self, x):
        ...
        # Streamed fusion to avoid massive intermediates
        R = prf_feat.shape[0]
        H = poly_feat.shape[1]
        P = poly_feat.shape[2]
        M = prf_feat.shape[3]
...
class XMLDataset:
    ...
    def __next__(self):
        ...
        max_labels = min(
            MAX_LABELS,
            max((len(self.Y[i]) for i in batch_indices), default=1)
        )
...
def run_benchmark(dataset_name: str = "all", method_filter: str = "all"):
    ...
    for ds_name in datasets_to_run:
        ...
        # Config
        # Mac GPU–safe batch sizes
        if jax.devices()[0].platform == "metal":
            BATCH_SIZE = 96   # M4
        else:
            BATCH_SIZE = 1024  # CUDA
        ...
        all_configs = [
            ...
            ('SLAY (Approx)', {'attention_type': 'slay', 'attention_kwargs': {'num_features': 16, 'num_quadrature_nodes': 2}}),
        ]
